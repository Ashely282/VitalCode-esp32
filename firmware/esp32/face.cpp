...
// Screen is assumed 128x64 (SH1106). Eyes are always drawn with
// fillRoundRect so they read as "rounded-square", never circles
// or sharp rectangles, matching the AIBI-style reference.
// ============================================================

// Layout constants (tweak here to restyle the whole face at once)
...
void Face::setExpression(Expression e) {
  currentExpression = e;
  expressionStartTime = millis();
  timedExpressionUntil = 0; // manual expression, stays until changed again
  animFrame = 0;
}
...
// style: 0=flat 1=smile(up) 2=frown(down) 3=talking "O" 4=tiny neutral
void Face::drawMouth(int style) {
  ...
  switch (style) {
    case 0: // flat
      ...
      break;
    case 1: { // smile - simple upward curve using two line segments
   ...
   case 3: // talking "O" - small filled circle, mouth is the one exception to rounded-square rule
      display->fillCircle(cx, MOUTH_Y + 2, 4, SH110X_WHITE);
      break;
    case 4: // tiny neutral dot-dash
...
void Face::drawHeart(int cx, int cy) {
  // Small pixel heart made of two circles + a triangle, memory-light.
...
void Face::drawPillIcon(int x, int y) {
  // Simple two-tone capsule: outline rounded rect split by a line.
...
void Face::drawHappy() {
  // Eyes slightly shorter (squinted-happy) + bounce via animFrame
  int bounce = (animFrame / 5) % 2 == 0 ? 0 : -2; // gentle up/down bob
...
void Face::drawSad() {
  // Eyes shifted down slightly, top corners less rounded = "droopy"
...
void Face::drawSleep() {
  // Eyes as thin closed lines (rounded), plus animated "Zzz"
  drawEye(LEFT_EYE_X, EYE_Y + 12, EYE_W, 4, 2);
  drawEye(RIGHT_EYE_X, EYE_Y + 12, EYE_W, 4, 2);
  drawMouth(4);
// Zzz drifts up and fades by looping animFrame
...
void Face::drawThinking() {
  // One eye shifted up/sideways to suggest looking off in thought
...
oid Face::drawExcited() {
  ...
  drawMouth(1);
  // sparkle - tiny blinking dots near top corners
  if ((animFrame / 6) % 2 == 0) {
...
void Face::drawAngry() {
  // Rounded-square eyes tilted "inward" using drawTriangle brows
  // over the top of each eye to suggest angled eyebrows, staying cute.
  drawEye(LEFT_EYE_X, EYE_Y + 2, EYE_W, EYE_H - 4, 5);
  drawEye(RIGHT_EYE_X, EYE_Y + 2, EYE_W, EYE_H - 4, 5);
  // eyebrow lines angled toward the center (the "angry" cue)
...
void Face::drawLove() {
  drawHappy(); // reuse happy eyes as the base
  drawHeart(64, 8);
}

void Face::drawDizzy() {
  // Eyes rotate: approximate with a spinning "swirl" of 4 dots per eye
  float angle = (animFrame * 20) * PI / 180.0;
  int cxL = LEFT_EYE_X + EYE_W / 2;
  int cxR = RIGHT_EYE_X + EYE_W / 2;
  int cy = EYE_Y + EYE_H / 2;
  drawEye(LEFT_EYE_X, EYE_Y, EYE_W, EYE_H, 6, false); // outline only
...
void Face::drawListening() {
  drawNeutral(); // base face
  // animated sound-wave bars under the chin as a "listening" indicator
  int base = 60;
  for (int i = 0; i < 3; i++) {
...
void Face::drawTalking() {
  drawEye(LEFT_EYE_X, EYE_Y, EYE_W, EYE_H);
  drawEye(RIGHT_EYE_X, EYE_Y, EYE_W, EYE_H);
  // mouth alternates open/closed to look like talking
  if ((animFrame / 4) % 2 == 0) {
    drawMouth(3);
  } else {
    drawMouth(0);
...
void Face::drawWink() {
  drawEye(LEFT_EYE_X, EYE_Y, EYE_W, EYE_H); // left stays open
  drawEye(RIGHT_EYE_X, EYE_Y + 12, EYE_W, 4, 2); // right closed (thin line)
  drawMouth(1);
}
...
oid Face::drawShaken() {
  // Big round-ish bright eyes (still rounded-square, high radius) + playful tongue
  drawEye(LEFT_EYE_X - 3, EYE_Y - 3, EYE_W + 6, EYE_H + 6, 12);
  drawEye(RIGHT_EYE_X - 3, EYE_Y - 3, EYE_W + 6, EYE_H + 6, 12);
  drawMouth(1);
  // playful tongue: small rounded rect sticking out of mouth
...
void Face::drawCharging() {
  // Circular progress ring made of arcs approximated via pixel plotting,
  // original design (not copying any device's proprietary graphic).
...
void Face::drawLoading() {
  // Simple rotating rounded-square "spinner" made of the eye primitive
  int cx = 64, cy = 28;
...
void Face::maybeAutoBlink() {
  // Only auto-blink during calm expressions; avoid interrupting e.g. sleep/wink.
...
void Face::update() {
  ...
  // advance shared animation frame counter at a fixed rate, non-blocking
  if (now - lastFrameTime >= ANIMATION_FRAME_MS) {
    animFrame++;
    lastFrameTime = now;
  }

  // auto-return from a timed expression (e.g. LOVE after medicine taken)
...
maybeAutoBlink();
  ...
  if (isBlinking) {
    // short blink animation: closed eyes for ~150ms, then reopen
    drawEye(LEFT_EYE_X, EYE_Y + 12, EYE_W, 4, 2);
    drawEye(RIGHT_EYE_X, EYE_Y + 12, EYE_W, 4, 2);
    drawMouth(0);
